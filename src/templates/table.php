	...
	<tbody>
		<?php foreach ($stats AS $url => $data) {
			...
			<tr<?= $isurl ? '' : ' class="minify__table-totalrow"'; ?>>
				...
				<td>Uncompressed</td>
				<td><?= number_format($data['input']); ?></td>
				<?php foreach ($data['minifiers'] AS $key => $item) {
					...
					<td rowspan="2">
						<?php if ($isurl) { ?>
							<a href="?action=code&amp;minifier=<?= htmlspecialchars($key); ?>&amp;url=<?= urlencode($url); ?>" target="_blank" class="icon-code">View Code</a>
						<?php } ?>
					</td>
				<?php } ?>
			</tr>
			<tr>
				<td>Gzipped</td>
				<td><?= number_format($data['inputgzip']); ?></td>
				<?php foreach ($data['minifiers'] AS $key => $item) {
					if ($item['irregular']) {
						$cls = ' class="minify__table--failed"';
